...
from backPacket.demoApp.sever.InformationSystem.WorkSchedule import CheckWorkScheduleSever
from backPacket.demoApp.tools.usefulTools import UsefulTools

routeKey = 'checkWorkSchedule'
api_route = Blueprint(routeKey, __name__) #固定部分，不要删除

"""
类名：checkWorkScheduleController
父类：baseController
...
class checkWorkScheduleController(baseController):
    def __init__(self):
    ...
    def checkWorkSchedule(key=1):
        ...
        try:
            ...
            workSchedule.setWorkStaffId(staffId)
            workSchedule.setStartTime(startTime)
            workSchedule.setEndTime(endTime)
            checkWorkScheduleSever = CheckWorkScheduleSever.CheckWorkScheduleSever()
            workSchedulePage = checkWorkScheduleSever.CheckWorkSchedule(workSchedule,staff)
            retMess = workSchedulePage
...
fullPath = checkWorkScheduleController().fullPath #固定部分，不要删除