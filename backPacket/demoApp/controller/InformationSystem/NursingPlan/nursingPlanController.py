...
from backPacket.demoApp.sever.InformationSystem.NursingPlan import NursingPlanSever
...
routeKey = 'nursingPlan'
api_route = Blueprint(routeKey, __name__) #固定部分，不要删除

"""
类名：nursingPlanController
父类：baseController
描述：查询护理计划Controller层实现
入参：无初始化入参
出参：无初始化出参
作者：马梓洋
完成时间：2023/3/30
...
class vitalTaskController(backPacket.demoApp.controller.baseController.baseController):
    ...
    def nursingPlan(key=1):
        ...
        try:
            ...
            nursingPlan = DepartNursing.DepartNursing()
            BeginDate=dataDict["BeginDate"]
            EndDate=dataDict["EndDate"]
            BeginDate,EndDate = UsefulTools().null2Time(BeginDate,EndDate,0,1)
            nursingPlan.setBeginDate(BeginDate)
            nursingPlan.setEndDate(EndDate)
            nursingPlanSever = NursingPlanSever.NursingPlanSever()
            nursingPlan.setStaff(staff)
            nursingPlan.setDepartId(departId)
            depart = nursingPlanSever.nursingPlanSever(nursingPlan)#调用sever层
            # 最终输出预处理
            depart.obj2Dict()
