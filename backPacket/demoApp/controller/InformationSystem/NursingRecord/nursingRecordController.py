...
from backPacket.demoApp.sever.InformationSystem.NursingRecord import NursingRecordSever
...
routeKey = 'nursingRecord'
api_route = Blueprint(routeKey, __name__) #固定部分，不要删除

"""
类名：nursingRecordController
父类：baseController
描述：查询护理记录Controller层实现
入参：无初始化入参
出参：无初始化出参
作者：马梓洋
完成时间：2023/4/1
...
class nursingRecordController(backPacket.demoApp.controller.baseController.baseController):
    def __init__(self):
    ...
    def nursingRecord(key=1):
        ...
        try:
            ...
            nursingRecord = DepartNursing.DepartNursing()
            BeginDate=dataDict["BeginDate"]
            EndDate=dataDict["EndDate"]
            BeginDate,EndDate = UsefulTools().null2Time(BeginDate,EndDate,0,1)
            nursingRecord.setBeginDate(BeginDate)
            nursingRecord.setEndDate(EndDate)
            nursingRecordSever = NursingRecordSever.NursingRecordSever()
            nursingRecord.setStaff(staff)
            nursingRecord.setDepartId(departId)
            depart = nursingRecordSever.nursingRecordSever(nursingRecord)
            # 最终输出预处理
            depart.obj2Dict()
...
fullPath = nursingRecordController().fullPath #固定部分，不要删除