...
from backPacket.demoApp.sever.manageSystem.UnwellLog.UpdateUnwellManageSever import UpdateUnwellManageSever
from backPacket.demoApp.sever.BaseSever.CheckStaffMessSever import CheckStaffMessSever
from backPacket.demoApp.tools.usefulTools import UsefulTools
from backPacket.demoApp.dto.Staff import Staff

routeKey = 'updateUnwellManage'
api_route = Blueprint(routeKey, __name__) #固定部分，不要删除

"""
类名：updateUnwellManageController
父类：baseController
...
class updateUnwellLogController(manageBaseController):
    def __init__(self):
    ...
    def checkExecute(key=1):
        ...
        try:
            ...
            unwellLog.setCreateId(modeKey)
            # 入参信息装载完毕，开始调用sever层
            updateUnwellManageSever = UpdateUnwellManageSever()
            retMess = updateUnwellManageSever.updateUnwellManage(unwellLog,staff,newStatusCode)
