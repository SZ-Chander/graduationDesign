...
from backPacket.demoApp.sever.manageSystem.WihteBoard import UpdateManageContentSever
from backPacket.demoApp.controller.manageSystem.manageBaseController import manageBaseController

routeKey = 'updateManageContent'
...
class updateManageContentController(manageBaseController):
    def __init__(self):
    ...
    def updateManageContent(key=1):
        ...
        try:
            ...
            whiteBoardContent.setTopMess(topMess)
            # 将dto装载金to
            ...
            # 信息装载完成，开始调用sever层
            updateManageContentSever = UpdateManageContentSever.UpdateManageContentSever()
            retMess = updateManageContentSever.updateManageContentSever(updateContentTO)
...
fullPath = updateManageContentController().fullPath #固定部分，不要删除