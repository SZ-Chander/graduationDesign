...
from backPacket.demoApp.sever.manageSystem.DepartWorkSchedule.InsertWorkScheduleSever import InsertWorkScheduleSever
from backPacket.demoApp.tools.usefulTools import UsefulTools

routeKey = 'insertWorkSchedule'
api_route = Blueprint(routeKey, __name__) #固定部分，不要删除

"""
类名：insertWorkScheduleController
...
class insertWorkScheduleController(manageBaseController):
    def __init__(self):
    ...
    def creatAccount(key=1):
        ...
        try:
            ...
            # createStaffId
            workSchedule.setCreateStaffId(userId)
            # workStaffId
            ...
            # DTO装载完成，开始进入Sever层
            insertWorkScheduleSever = InsertWorkScheduleSever()
            retMess = insertWorkScheduleSever.insertWorkSchedule(workSchedule)
...
fullPath = insertWorkScheduleController().fullPath #固定部分，不要删除