...
from backPacket.demoApp.sever.manageSystem.Audit import UpdateAssessAuditSever

routeKey = 'updateAssessAudit'
api_route = Blueprint(routeKey, __name__) #固定部分，不要删除

"""
类名：updateAssessAuditController
父类：baseController
描述：审核护理评估功能的Controller层实现
入参：无初始化入参
出参：无初始化出参
作者：马梓洋
完成时间：2023/4/25
...
class updateAssessAuditController(manageBaseController):
    def __init__(self):
    ...
    def updateAssessAudit(key=1):
        ...
        try:
            ...
            # 读取护理评估审核信息（更新计划，完成或同时两种模式）
            assess = NursingRecord.NursingRecord()
            updateKey = dataDict["updateKey"]
            if(updateKey == 1):# 用于更新“通过”
                assess.setAudit_A(1)
                assess.setAuditWhy_A("--无--")
            elif(updateKey == 2):# 用于更新“不通过”
                assess.setAudit_A(2)
                assess.setAuditWhy_A(dataDict['auditWhy'])
            ...
            # 打包完成
            updateAssessSever = UpdateAssessAuditSever.UpdateAssessAuditSever()
            respond = updateAssessSever.updateAssessAuditSever(departNursing,updateKey)
            if(respond == True):
                retMess = "护理评估审核成功"
            else:
                retMess = "护理评估审核异常，code:Error30{}\n请联系系统管理员".format(respond)
            # 代码块结束
...
fullPath = updateAssessAuditController().fullPath #固定部分，不要删除