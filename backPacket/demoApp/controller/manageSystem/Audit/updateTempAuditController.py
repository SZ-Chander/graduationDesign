...
from backPacket.demoApp.sever.manageSystem.Audit import UpdateTempAuditSever

routeKey = 'updateTempAudit'
api_route = Blueprint(routeKey, __name__) #固定部分，不要删除

"""
类名：updateTempAuditController
父类：baseController
描述：审核体温单功能的Controller层实现
入参：无初始化入参
出参：无初始化出参
作者：马梓洋
完成时间：2023/4/25
...
class updateTempAuditController(manageBaseController):
    def __init__(self):
    ...
    """
    功能阐述:更新体温单信息controller层实现
    @:date 2023/4/25
    @:author 马梓洋
    """
    @api_route.route('/', methods=['POST'])
    def updateTempAudit(key=1):
        """
        ...
        try:
            ...
            if(updateKey == 1):# 用于更新“通过”
                ...
                temp.setAudit(2)
                temp.setAuditWhy(dataDict['auditWhy'])

            else:
                raise ValueError("非法数据，更新代码错误")

            temp.setTempTableId(dataDict['TempTableId'])
            # 信息读取完毕，将工作人员信息和护理计划更新信息打包
            departNursing = DepartNursing.DepartNursing()
            departNursing.setNursingList(temp)#虽然变量名是list，但这里实际上不是list，而只是一个实例化对象
            departNursing.setStaff(staff)
            departNursing.setDepartId(dataDict['departId'])
            # 打包完成
            updateTempSever = UpdateTempAuditSever.UpdateTempAuditSever()
            respond = updateTempSever.updateTempAuditSever(departNursing,updateKey)
            if(respond == True):
                retMess = "体温单审核成功"
            else:
                retMess = "体温单审核异常，code:Error30{}\n请联系系统管理员".format(respond)
            # 代码块结束
...
fullPath = updateTempAuditController().fullPath #固定部分，不要删除