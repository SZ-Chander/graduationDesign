...
from backPacket.demoApp.dao.manageSystem.DepartWorkSchedule.UpdateWorkScheduleDao import UpdateWorkScheduleDao
"""
类名：UpdateWorkScheduleSever
描述：创建新账号功能Sever层实现类
入参：无初始化入参
出参：无初始化出参
作者：常舜志
    ...
    def updateWorkSchedule(self,workSchedule):
        try:
            ...
            if(workStaffDepartId in staff.getDepartCode()):
                ...
                updateWorkScheduleDao = UpdateWorkScheduleDao()
                retData = updateWorkScheduleDao.updateWorkSchedule(workSchedule)

                if(retData == 1):
                    return "排班信息修改成功！"
                elif(retData == -2):
                    raise ValueError("您不能修改已经结束的排班信息！")
                elif (retData == -1):
                    raise ValueError("排班开始时间不能早于当前时间！")
                else:
